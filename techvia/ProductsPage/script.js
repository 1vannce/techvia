...
function renderProducts(cat = "all", search = "") {
    ...
    list.forEach((p, index) => {
        ...
        card.innerHTML = `
            ...
            <div class="product-info">
                ...
                <div class="product-actions">
                    <button class="btn-cart" data-product-id="${p.id}">Add to Cart</button>
                    <button class="btn-details" data-product-id="${p.id}">View Details</button>
                </div>
            </div>
        `;
        ...
        card.querySelector(".btn-details").onclick = () => {
            window.location.href = `../ProductDetailsPage/index.html?id=${p.id}`;
        };
        
        card.querySelector(".btn-cart").onclick = (e) => addToCart(p.id, e);
    });
}
...
function updateCartCounter() {
    ...
    if (cartCountEl) {
        ...
        if (cartEl) {
            cartEl.textContent = `🛒 Cart (${totalItems})`;
        }
    }
}
